...
`any-switch` 是一个通用的本地 profile / state 切换工具。它把某个本地 App 或 CLI 的一组可管理状态建模为 profile，并通过声明式 App Definition，把 profile 捕获、渲染或回放到具体 target，例如 JSON 子树、TOML 路径、文件、Keychain entry 或环境变量块。
...
| `any-switch restore-target <app> <backup-id>` | 从防御性备份恢复目标位置。交互式 TTY 输入 `yes` 确认，脚本 / CI 使用 `--yes` |
| `any-switch doctor [<app>]` | 检查路径、权限、字段完整性、当前平台 capture source 存在性、进程状态、Keychain 可访问性 |
...
C. **Claude 并发写 `~/.claude.json` 实测**：Claude Code 运行时观察 `~/.claude.json` 的写频率与字段，识别哪些字段会和 `$.oauthAccount` 同一次写入，并记录顶层 `userID` 是否独立变化。同时采样 Claude Code 自己写出的 JSON 格式参数（indent 字符与宽度、是否 trailing newline、是否 minified、顶层 key 顺序），把结果固化为 §9.4 采样算法的默认值或 doctor 校验项；如果 Claude Code 写出的格式与 §9.4 默认值不同，说明 any-switch 必须严格沿用读入时的采样而不能落回默认。
...
`any-switch` 第一版是一个**本地 profile / state 切换器**。Claude Code / Codex 的账号和凭据切换是 MVP 的首个内置应用域，因为它同时覆盖了静态字段渲染、OAuth 动态 capture、secret 脱敏、备份回滚和 App Definition 扩展等核心能力。
