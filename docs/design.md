...
- `--assume-app-stopped`：仅用于 `oauth_capture` 进程探测命中但用户确认为误报时的逃生口（见 §11.4）。必须显式拼写完整，没有短形式；必须配合 `--yes` 在非交互式生效，或在交互式 TTY 下接受额外的二次确认。触发时会把探测到的 PID 列表写入 `history.jsonl` 的 `warnings`。
...
- **oauth_capture kind：命中默认强制拒绝，`--allow-running` 不生效**。理由：App 运行时会刷新 token，原子性无法保证，且我们 writeback 的瞬间 App 也可能在写。

**进程探测误报的逃生口**：进程名匹配本质是粗粒度信号，会有误报（同名 wrapper / launcher / 孤儿进程）和漏报（用户用别名启动）。对 oauth_capture 命中时：
...
漏报方向无法用 CLI flag 防御——这是 App Definition `process_probe` 表达力的限制。第一类用户教育：oauth_capture 切换前用户应主动 `Quit Claude.app` / `kill claude` 并通过 `switch doctor` 二次确认；用户必须理解 §11.4 默认拒绝是"对漏报偏保守"的设计选择，而非额外强制保证。Phase 2 可考虑给 `process_probe` 增加 `lockfile` / `socket` 等次级 handler 来收紧漏报。
...
| `LockBusy` | 另一个写操作正在执行 |
| `AppRunning` | 目标 App 进程在运行；oauth_capture 默认拒绝（仅 `--assume-app-stopped` + `--yes` 可逃生），其他 kind 默认拒绝（可 `--allow-running` 跳过）；错误信息含探测到的 PID + 命令行 + 启动时间 |
...
11. `use` 在检测到目标 App 进程运行时默认拒绝；env_injection / file_template 可 `--allow-running` 跳过；**oauth_capture 不接受 `--allow-running`**。`AppRunning` 错误必须列出探测到的 PID + 命令行 + 启动时间；仅在用户对每个 PID 都能自证非目标 App 时，可叠加 `--assume-app-stopped` + `--yes`（非交互式）或额外二次确认（交互式）跳过，逃生记录写入 history `warnings`。
...
23. Codex `config.toml` 只能修改 Definition 声明的 managed TOML paths，必须保留 `mcp_servers` / `projects` / `plugins` 等未知配置。验收以 TOML AST 比对（语义等价 + managed paths 外 key 顺序一致），不要求字节相等；datetime offset 表示、inline table 内部空白与等价语法变体的 round-trip 差异允许存在。
