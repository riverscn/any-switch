...
- **进程互斥强制**：任何涉及 OAuth capture 读写的操作如果检测到目标 App 在运行，**拒绝**执行——`--allow-running` 不生效。这包括切到 OAuth profile、切走当前 active OAuth profile 前的 writeback，以及 writeback-only self-use。理由是 App 运行时会刷新 token，原子性无法保证。
...
manifest 同时保存原始模板（`path` / `account`，用于展示）和备份创建时解析出的真实目标（`resolved_path` / `resolved_account`，用于恢复）。`restore-target` 只信任 manifest 中声明的 resolved target spec；目录里存在但 manifest 未引用的文件一律忽略并 warning。manifest 缺失、resolved target 不在 home 内、落入 `SWITCH_CLI_HOME`、handler 不再受支持，或 target spec 无法通过当前安全校验时，不执行恢复，报 `BackupInvalid`。恢复不重新读取当前环境变量来改变目标位置；如果用户希望恢复到新的 `CODEX_HOME` / `CLAUDE_CONFIG_DIR`，应先在目标 App 外部迁移配置，再重新 `import-current`。
...
| `switch use <id>` | 切换到指定 profile（oauth_capture 会先写回当前活动 profile 的 capture，不修改 profiles.yaml）。当 `<id>` 就是当前活动 oauth_capture profile 自身时，命令退化为"只跑 writeback"，可作为定期保护 capture 的安全操作（详见 §8.3 short-circuit）|
...
- `--allow-running`：在目标 App 进程运行时仍允许切换。**对任何涉及 `oauth_capture` 读写的操作不生效**——包括切到 OAuth profile、从当前 active OAuth profile 切走时的 writeback、以及 `switch use <当前 OAuth profile>` 的 writeback-only 操作；这些场景永远要求 App 退出。
...
if state/pending-switch/<app>.json exists:
    run pending-switch recovery first:
      ...
      - if pending.operation == "use" and live targets already match pending.to_profile
          -> finish active.json/history bookkeeping and delete journal
      - if pending.operation == "restore-target" and live targets already match pending.restore_from_backup_id
          -> finish history bookkeeping only, do not touch active.json, delete journal
...
previous = state.active_profiles[app] ? load profile(state.active_profiles[app]) : null
oauth_io_required =
...
- `status <app>` 报 `no-active`。
- 用户可以 `switch import-current <app> <new-name>` 把当前 live state 捕获成新 profile（identity 和已有 profile 重合时 `import-current` 会询问是否刷新现有 capture，参见 §8.2）。
- 用户可以 `switch use <existing-id>` 切回任意已注册 profile；该次 `use` 因 active 为 null，**不执行 writeback**（没有"当前活动 profile"可写回），但仍走完整的 defensive backup → apply → verify → bookkeeping。
...
- **任何涉及 oauth_capture 读写的操作：命中默认强制拒绝，`--allow-running` 不生效**。这包括切到 OAuth profile、从当前 active OAuth profile 切走时的 writeback，以及 `switch use <当前 OAuth profile>` 的 writeback-only 操作。理由：App 运行时会刷新 token，原子性无法保证，且我们 writeback 的瞬间 App 也可能在写。
...
11. `use` 在检测到目标 App 进程运行时默认拒绝；纯 env_injection / file_template 操作可 `--allow-running` 跳过；**任何涉及 oauth_capture 读写的操作都不接受 `--allow-running`**。`AppRunning` 错误必须列出探测到的 PID + 命令行 + 启动时间；仅在用户对每个 PID 都能自证非目标 App 时，可叠加 `--assume-app-stopped` + `--yes`（非交互式）或额外二次确认（交互式）跳过，逃生记录写入 history `warnings`。
