...
`switch-cli` 是一个本地命令行工具，用来管理 Codex 和 Claude Code 等 AI CLI 工具的账号配置并在它们之间快速切换。

工具不接入任何第三方登录接口。它把每个账号建模为一条**结构化记录**：
...
```text
我在 Codex / Claude Code 中需要在多个账号或服务端点之间切换，
...
```bash
# 把当前 Claude Code 的 Anthropic OAuth 登录捕获为一条账号 (oauth_capture)
switch import-current claude anthropic-work

# 添加一个 GLM 第三方代理账号 (env_injection)
...
# Codex 当前是 ChatGPT OAuth 登录，捕获为账号 (oauth_capture)
switch import-current codex chatgpt-work

switch list
switch status

# 长期未切到该账号、refresh token 被服务端旋转作废时，重新登录并回灌
...
- 账号建模为结构化记录，可由编辑器手编，也可由 `import-current` 自动捕获。
- 支持四种凭据投递方式（kind）：
...
- `oauth_capture` 在切换时执行**双向写回**——切换前先把当前活动账号的最新 token 回写到它自己的 capture 中，再加载目标账号，避免 refresh token 旋转造成静默失效。
...
- 校验账号在服务端是否仍可用——`reauth` 走 App 自己的 `/login` 流程，不绕过它。
- GUI 和 TUI。
...
工具管理的基本单元是 `account record`：一个有 `id` / `app` / `kind` / `fields` 或 `capture` 的结构化对象。结构化记录的好处：
...
每个 App Module 声明自己支持哪几种 kind，并提供"账号字段 / capture → 目标文件内容"的转换器。
...
- **写回先行**：切换前先把当前 Keychain / 凭据文件的最新内容回写到当前活动账号的 capture，再写入目标账号。
...
- account id / name / app / kind / created_at
...
用户管理的一条账号记录。至少包含 `id` / `app` / `kind` / `name`，其余字段随 kind 变化。account id 全局唯一。

### 4.3 Kind

账号的凭据投递方式。MVP 四种：`env_injection`、`file_template`、`oauth_capture`、`opaque_capture`（保留）。
...
`env_injection` 和 `file_template` 类账号的语义字段集合：
...
`oauth_capture` 账号的不变身份指纹。**不含 secret，用于校验和展示**：
...
`oauth_capture` 和 `opaque_capture` 账号的 blob 引用。每个 capture 描述若干 source，每个 source 标注类型、存储位置、sha256、可选的平台限定。
...
freshness:
  ...
  last_writeback_at: "2026-05-23T15:30:00Z"   # 最近一次切走该账号时的 writeback 时间
```

`expires_at` 只用于切换前的过期提示，**不**用于判断该账号能否切（access_token 过期不代表 refresh_token 失效）。`last_writeback_at` 用于审计：如果一个账号从来没被切换过、`captured_at` 已经很久远，refresh_token 大概率已被 server 端旋转作废，切过去会失败。
...
切换前对所有将被改写的目标位置自动建立的备份。它是防御性的，不是账号备份——目的是用户在目标文件里的手工改动（MCP 配置、自定义 Codex profile 等）丢失后可以恢复。

backup 不出现在 account 列表里。Keychain entry 和 JSON 子树同样要进 backup（前者以 JSON 文件形式落地，后者以原值落地）。
...
```text
App: claude
Account: claude-anthropic-work  (oauth_capture)
Identity: work@example.com  org=Personal  uuid=5f3e...

Writeback (current active: claude-anthropic-personal):
...
```text
config: ~/.config/switch-cli/config.yaml   # 用户配置 + 账号记录（source of truth）
data:   ~/.local/share/switch-cli/         # 捕获 blob 和防御性备份
state:  ~/.local/state/switch-cli/         # 活动账号指针和历史
```
...
```json
{
  ...
  "operation": "use",
  "app": "claude",
  "from_account": "claude-anthropic-personal",
  "to_account": "claude-anthropic-work",
  "kind": "oauth_capture",
  "writeback_ok": true,
  "verify_ok": true,
...
`config.yaml` 是账号记录的 source of truth，同时承载 CLI 行为偏好。
...
- 每条 account 的 `schema_version` 标识该 `app + kind` 组合的字段 schema 版本，可独立演进。
...
| `switch list [<app>]` | 列出已注册账号（脱敏） |
| `switch show <id>` | 查看单个账号的元数据、identity、非敏感字段 |
| `switch add <app> <name> [--id <id>] [--kind <kind>] [--field k=v ...]` | 手动添加 env_injection / file_template 账号 |
| `switch edit <id>` | 用 `$EDITOR` 打开账号的 yaml 片段编辑 |
| `switch remove <id>` | 删除账号；同时清理 captures/<id>/ |
| `switch import-current <app> <name> [--id <id>]` | 从 App 当前状态自动捕获一条账号 |
| `switch use <id>` | 切换到指定账号（oauth_capture 会先写回当前活动账号）|
| `switch use <id> --dry-run` | 只打印 plan，不写入 |
| `switch reauth <id>` | 引导用户重新登录 App 并把新凭据回灌到该 oauth_capture 账号 |
| `switch status [<app>]` | 显示每个 App 的活动账号、是否 drift |
...
- `--allow-running`：在目标 App 进程运行时仍允许切换。**对 `oauth_capture` 不生效**——切 OAuth 账号永远要求 App 退出。
...
append account to config.yaml (atomic write)
```
...
for oauth_capture drafts:
  ...
  check identity.account_uuid against existing accounts: if match, ask user
    whether to refresh that account's capture instead of creating new
...
on confirm:
  ...
  - write account to config.yaml (atomic)
release lock
```
...
resolve target account by id
acquire app lock
detect target app process running:
  - if kind == oauth_capture: refuse, ignore --allow-running
  - else: refuse unless --allow-running

# Step A: writeback current active (oauth_capture only)
if state.active[app] exists:
    previous = load account(state.active[app])
...
# Step B: build plan
build plan from target account (kind-specific render or load from captures/)
...
for kind == oauth_capture:
    ...
    must equal target.identity.account_uuid
...
if target.kind == oauth_capture and freshness.captured_at is older than
...
verify identity.account_uuid matches the existing record
  - if mismatch: ask user whether this is a different account (suggest add as new)
  - if match: update capture/* bytes and sha256, update freshness.captured_at
atomic write config.yaml
state.active[app] = <id>  (reauth implies activation)
release lock
```
...
load config + state
for each app:
  read active id from state/active.json
  if no active                              -> no-active
  resolve account
  ...
  for oauth_capture:
    ...
    compare to account.identity -> matched / drifted
...
| `drifted` | 活动账号 id 已知，但实际状态与预期不符 |
| `missing` | required target 不存在 |
| `no-active` | state 里没有该 App 的活动账号 |
...
2. 删除上一活动账号 `managed_env_keys` 中的所有键。
3. 写入当前账号渲染出的 env 键。
...
**Env 覆盖**：`CLAUDE_CODE_OAUTH_TOKEN` 走 setup-token 长期凭据，仅 inference-only，不适合日常账号切换，MVP 不纳入管理。
...
- import_current：从当前状态推断账号草稿（可能返回多个候选）。
...
pub trait AppModule {
    ...
    fn render(&self, account: &Account, current: &TargetState)
        -> Result<RenderedTargets, RenderError>;
    fn restore(&self, account: &Account, captures: &CaptureBytes)
        -> Result<RenderedTargets, RenderError>;
    ...
    fn import_current(&self, ctx: &Ctx) -> Result<Vec<AccountDraft>, ImportError>;
...
- verify：切换后从 `~/.claude.json` 读 `$.oauthAccount.accountUuid`，必须等于 `account.identity.account_uuid`。
...
- 目标文件：
  ...
  - `~/.codex/config.toml`：渲染当前账号对应的 `[model_providers.<name>]` 段及活动 model 字段。
- 字段：
...
允许打印：account id / name / app / kind / created_at、非敏感字段、`identity` 块、目标位置 metadata、capture source 的 metadata、sha256 前缀。
...
1. **切换时写回**：切走当前活动 oauth_capture 账号前，先把最新 Keychain / 凭据文件内容回灌到该账号的 capture。这样只要用户在两个账号之间循环切换，每个 capture 都保持最新。
...
| `AccountNotFound` | 指定 id 的账号不存在 |
| `AccountExists` | `add` 时 id 已被占用 |
...
| `WritebackFailed` | oauth_capture 切换前的写回当前活动账号失败 |
...
hint: target account's capture may be from a different Anthropic account.
      run `switch reauth claude-anthropic-work` to refresh.
```

## 13. MVP 验收标准

1. 能用 `add` 手动创建 Claude `env_injection` 账号、Codex `file_template` 账号。
2. 能用 `import-current` 在 macOS 上从当前 Keychain + `~/.claude.json` 自动捕获一条 Claude `oauth_capture` 账号；identity 字段（accountUuid / email / organizationName）正确提取。
3. 能用 `import-current` 在 Linux 上从 `~/.claude/.credentials.json` + `~/.claude.json` 自动捕获 Claude `oauth_capture` 账号。
...
5. 能在已注册账号之间切换：
   ...
   - 切换后重启 App 实测确认账号生效（macOS 与 Linux 各做一次）。
...
13. `remove` 能删除账号；同时清理 `captures/<id>/`。
...
- `daemon` 模式：常驻监听 Keychain / credentials.json 变化，实时回写当前 active 账号的 capture，进一步降低旋转失效风险。
...
如果账号切换之外出现明确需求，再考虑通用 state / plan / apply 模型、外部模块协议、插件系统、trust / allow 机制。这些能力不应反向污染账号切换 MVP。

## 15. 当前设计结论

`switch-cli` 第一版是一个**账号配置管理与切换工具**，特别认真地对待 OAuth 这类动态凭据。

最重要的边界：

1. 账号是结构化记录（`id` / `app` / `kind` / `fields` 或 `identity + capture`），不是 opaque 文件快照。
2. 四种 kind（`env_injection` / `file_template` / `oauth_capture` / `opaque_capture`）覆盖现实世界的主要凭据投递方式。
