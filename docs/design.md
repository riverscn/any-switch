...
- 解析 OAuth blob 内部结构（JWT 签名校验、scopes 语义判断、access token 过期时间等）。
...
- **陈旧感知**：capture 记录 `captured_at` 和 `last_writeback_at`，切换时如果检测到 capture 长期没有被写回，警告用户该 capture 可能已经失效。
...
### 4.8 Capture Meta（oauth_capture 专用）
...
`capture_meta` 只用于本地审计和陈旧提示，不用于判断该 profile 能否切。判断依据是 `last_writeback_at ?? captured_at`：如果一个 profile 从来没被切换过、`captured_at` 已经很久远，或最近一次写回很久远，refresh_token 大概率已被 server 端旋转作废，切过去可能失败。
...
```text
~/.switch-cli/
  profiles.yaml                    # 用户主入口：profile 注册表 + preferences
...
| `~/.switch-cli/profiles.yaml` | **profile 注册表 + CLI 偏好** | switch-cli 在 add / edit / remove / use 等命令中维护，也可被用户手编 |
| `~/.switch-cli/captures/<id>/` | oauth_capture profile 的 blob 内容 | switch-cli 自动维护（import-current / writeback）|
...
3. **动态凭据的清单**（oauth_capture）：secret blob **不**在 profiles.yaml 里。这里只放：
   ...
   - `capture.sources`：每个 source 的 type / 目标位置 / `stored_as` / `sha256`
   - `capture_meta`：`captured_at` / `last_writeback_at`

   实际的 OAuth blob 在 `~/.switch-cli/captures/<id>/` 下作为独立文件存。oauth_capture 切换时 profiles.yaml 只更新对应的 `sha256` 和 `capture_meta` 字段，diff 体量小。
...
OAuth blob、防御性备份、活动 profile 指针、操作历史等都属于工具自动维护的运行时状态，分别放在 `captures/` / `backups/` / `state/` / `locks/` 子目录下，不与 `profiles.yaml` 混在同一文件里。
...
- 读到旧 `schema_version` → 自动迁移到新版本；迁移失败时备份原 profiles.yaml 并以只读模式运行。
...
- `extensions` 是开放对象。
- `oauth_capture` 必须含 `identity` 和 `capture.sources`，可选 `capture_meta`。
...
on confirm:
  ...
  - write profile to profiles.yaml (atomic)
release lock
```
...
if state.active_profiles[app] exists:
    ...
    if previous.kind == oauth_capture:
        for each source in previous.capture.sources (current platform):
            ...
            update sha256 in profiles.yaml
        update previous.capture_meta.last_writeback_at = now
...
if target_profile.kind == oauth_capture and
   (capture_meta.last_writeback_at ?? capture_meta.captured_at) is older than
   oauth_stale_warn_days, warn user that refresh might fail and suggest
   verifying the App manually after switch. If the App reports auth failure,
   resolve that state outside switch-cli, then run `switch import-current`
   to capture or update the profile.
```
...
`use` / `import-current` / `restore-target` 持有 App 锁。`add` / `edit` / `remove` 持有 config 锁。`list` / `show` / `status` / `doctor` 无锁读取。
...
**Codex 待实证**：固化 Codex 系统预置 Definition 前必须先 `cat ~/.codex/auth.json` 看实际 schema，识别 OAuth blob 的可识别字段（如 `tokens.access_token` / `tokens.refresh_token` / `tokens.id_token` 等），并实测 refresh token 是否旋转。
...
| `toml_template` | 用字段模板渲染 TOML 文件或 TOML 子表 |
...
- 目标文件：
  - `~/.codex/auth.json`：含 API key 的 JSON（具体 schema 由系统预置 Definition 和 fixture 测试锁定）。
...
- identity 提取：从 `auth.json` 中识别 OAuth blob 字段（如 `tokens.id_token` 的 JWT payload `sub` / `email`）；具体字段实证后写入系统预置 Definition 和 fixture。
...
- 读 `auth.json`：
  ...
  - 两种字段共存（不太可能但需防御）→ 报错。
- 系统预置 Definition 固化前必须实测当前 Codex 版本的 auth.json 实际 schema，把识别规则写入 Definition 或对应 handler fixture，避免文档与实现脱节。
...
- 对 oauth_capture Definition：identity 提取和 capture_meta 更新 fixture 测试。
- 不输出 secret 字段值的脱敏测试。
- 路径展开和 home 边界测试。
...
2. **失效边界**：如果 capture 长期不被切到（即 refresh_token 已老化），用户切过去时 App 会在首次 refresh 时失败。CLI 基于 `capture_meta.last_writeback_at ?? capture_meta.captured_at` 提前警告，但不提供登录修复能力；用户在目标 App 外部恢复可用状态后，再运行 `switch import-current` 捕获或更新 profile。
...
- 读到旧 `schema_version` → 自动迁移；失败时备份原 profiles.yaml 为 `profiles.yaml.bak.<timestamp>` 并只读运行。
...
| `CaptureLikelyStale` | capture_meta 显示 capture 已超过保守阈值，提示该 capture 可能需要外部恢复后重新 import-current |
...
15. profiles.yaml / apps.d/ / overrides.d/ / captures/ / backups/ 权限不正确时启动有警告。
...
4. 用户可以手编 profiles.yaml（除 `oauth_capture` 外），也可以 `import-current` 自动捕获当前状态。
