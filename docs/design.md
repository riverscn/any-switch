...
**Capture 不跨平台 portable**：同一账号的 macOS Keychain entry value 与 Linux `~/.claude/.credentials.json` 内容结构相近但语义来源不同（系统 secret store vs. 应用文件），MVP 不做平台间字节迁移。把 profile 复制到新平台后，必须重新运行 `switch import-current` 在该平台上重新捕获 capture——identity（accountUuid 等）可用于在新机器上确认捕获到的是同一账号。
...
if state.active_profiles[app] exists:
    ...
    if previous.kind == oauth_capture:
        # B1. 先把所有 source 当前 bytes 全部读入内存
...
3. 能用 `import-current --kind auto` 在 Linux 上从 `~/.claude/.credentials.json` + `~/.claude.json` 自动识别并捕获 Claude `oauth_capture` profile。
4. 能用 `import-current --kind auto` 从 file-backed Codex `~/.codex/auth.json` 识别 API-key 模式 → `file_template`，或 ChatGPT OAuth 模式（`auth_mode == "chatgpt"` + `tokens.refresh_token`）→ `oauth_capture`；Codex keyring credential store 在 MVP 中明确报 `CredentialStoreUnsupported`。
